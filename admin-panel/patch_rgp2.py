...
upload_pattern = re.compile(r'const uploadedUrls = \[\];.*?setForm\(\{ \.\.\.form, challanImageUrls: newUrls, challanImageUrl: newUrls\[0\] \|\| \'\' \}\);', re.DOTALL)
...
content = upload_pattern.sub(upload_new, content)

# 2. Update modal attachments list
modal_pattern = re.compile(r'\{\(form\.challanImageUrls \|\| \[form\.challanImageUrl\]\)\.filter\(Boolean\)\.map\(\(url, i\) => \(.*?<X className="w-4 h-4" />.*?<\/button>.*?<\/div>.*?\)\)\}', re.DOTALL)
...
th_pattern = re.compile(r'<th className="table-header">Description</th><th className="table-header">Sent Date</th>')
th_new = '''<th className="table-header">Description</th><th className="table-header">Attachments</th><th className="table-header">Sent Date</th>'''
content = th_pattern.sub(th_new, content)

# 4. Update table row
td_pattern = re.compile(r'<td className="table-cell text-\[var\(--text-muted\)\] text-xs">\s*<div className="flex items-center gap-2">\s*\{r\.challanNumber \|\| \'-\'\}\s*\{\(r\.challanImageUrls\?\.length > 0 \|\| r\.challanImageUrl\) && \(\s*<a href=\{r\.challanImageUrls\?\.\[0\] \|\| r\.challanImageUrl\} target="_blank" rel="noreferrer" title=\{`View \$\{r\.challanImageUrls\?\.length > 1 \? \'Attachments\' : \'Attachment\'\}`\} className="text-teal-500 hover:text-teal-700 bg-teal-50 p-1 rounded flex items-center gap-1">\s*<ImageIcon className="w-3\.5 h-3\.5" />\s*\{r\.challanImageUrls\?\.length > 1 && <span className="text-\[10px\] font-bold">\{r\.challanImageUrls\.length\}</span>\}\s*</a>\s*\)\s*\}\s*</div>\s*</td>\s*<td className="table-cell text-\[var\(--text-muted\)\] text-xs max-w-\[200px\] truncate">\{r\.description \|\| \'-\'\}</td>\s*<td className="table-cell text-\[var\(--text-muted\)\] text-xs">', re.DOTALL)
