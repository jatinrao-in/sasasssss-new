...
export function usePushNotifications() {
  ...
  useEffect(() => {
    ...
    const isMemberPath = memberPaths.includes(location.pathname);

    if (userData && isMemberPath && getNotificationPermission() === 'default') {
    ...
    const unsubscribe = onMessage(messaging, async (payload) => {
      ...
      try {
        ...
        await swReg.showNotification(title, {
          ...
          data: { ...payload.data, url: URL_MAP[type] || '/pwa/dashboard' },
          vibrate: [200, 100, 200],
        });
      } catch {
