...
export default function DashboardPage() {
 ...
 const summaryCards = [
 { label: 'Open Tasks', value: openTasks.length, icon: ClipboardList, color: 'bg-teal-50 text-teal-600', iconBg: 'bg-teal-100', onClick: () => navigate('/pwa/tasks') },
 { label: 'Overdue', value: overdueTasks.length, icon: AlertTriangle, color: 'bg-red-50 text-red-600', iconBg: 'bg-red-100', onClick: () => navigate('/pwa/tasks') },
 { label: 'Follow-Ups', value: openFollowups.length, icon: CalendarClock, color: 'bg-amber-50 text-amber-600', iconBg: 'bg-amber-100', onClick: () => navigate('/pwa/follow-ups') },
 { label: 'Payments', value: pendingPayments.length, icon: CreditCard, color: 'bg-blue-50 text-blue-600', iconBg: 'bg-blue-100', onClick: () => navigate('/pwa/payments') },
 ];
 ...
 <Card key={i} variant="glass" className="min-w-[140px]"><CardContent className="p-3.5"><div className="h-9 w-9 bg-gray-200 rounded-lg animate-pulse mb-2.5" /><div className="h-6 w-10 bg-gray-200 rounded animate-pulse mb-1" /><div className="h-3 w-16 bg-gray-100 rounded animate-pulse" /></CardContent></Card>
 )) : summaryCards.map((card, idx) => {
 const Icon = card.icon;
 return (
 <Card key={idx} variant="glass" className="min-w-[140px] cursor-pointer hover:shadow-card-hover transition-shadow" onClick={card.onClick}>
 ...
 <button onClick={() => navigate('/pwa/tasks')} className="text-sm text-teal-600 font-medium flex items-center gap-0.5">View All <ChevronRight className="h-4 w-4" /></button>
 </div>
 ...
 <button onClick={() => navigate('/pwa/follow-ups')} className="text-sm text-teal-600 font-medium flex items-center gap-0.5">View All <ChevronRight className="h-4 w-4" /></button>
 </div>
 <div className="space-y-2.5">
 ...
 return (
 <Card key={project.id} className="min-w-[240px] max-w-[280px] cursor-pointer hover:shadow-card-hover transition-shadow" onClick={() => navigate(`/pwa/tasks?projectId=${project.id}`)}>
 <CardContent className="p-4">
