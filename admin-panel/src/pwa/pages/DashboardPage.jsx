...
export default function DashboardPage() {
 ...
 const getGreeting = () => {
  ...
  const summaryCards = [
    { label: 'Open Tasks', value: openTasks.length, icon: ClipboardList, color: 'bg-rose-50 text-[#E23744]', iconBg: 'bg-rose-100/60', onClick: () => navigate('/pwa/tasks') },
  ...
  return (
    <div className="pwa-dashboard-container pb-8 relative">
      {/* Interactive Glowing Aura Blobs (Zomato-inspired Premium Rose/Amber Glow) */}
      <div className="absolute top-[-5%] right-[-10%] w-72 h-72 rounded-full bg-rose-400/8 blur-[90px] pointer-events-none z-0" />
      <div className="absolute bottom-[25%] left-[-10%] w-72 h-72 rounded-full bg-amber-300/8 blur-[100px] pointer-events-none z-0" />
      
      <div className="relative z-10 space-y-5">
        {/* Premium Welcome Banner (Vibrant Crimson Gradient) */}
        <div className="px-4 pt-4">
          <div className="bg-gradient-to-br from-[#E23744] via-[#F43F5E] to-[#B91C1C] rounded-3xl p-5 text-white shadow-md relative overflow-hidden animate-slide-up">
            ...
            <div className="relative z-10 flex flex-col justify-between h-full">
              <div>
                <span className="text-[10px] uppercase font-bold tracking-wider text-rose-100 bg-rose-900/30 px-2.5 py-1 rounded-full">{getGreeting()}</span>
                <h1 className="text-2xl font-bold mt-2.5 leading-tight">
                  {userData?.name || 'Team Member'}
                </h1>
                <p className="text-xs text-rose-100/85 mt-1">Saya Industrial Team Portal</p>
              </div>
              
              <div className="mt-5 pt-3.5 border-t border-white/10 flex items-center justify-between text-xs text-rose-100">
              ...
              summaryCards.map((card, idx) => {
                ...
                return (
                  <div 
                    ...
                    className="bg-white border border-slate-100/85 rounded-2xl p-4 flex flex-col justify-between shadow-sm active:scale-95 transition-all duration-150 cursor-pointer hover:border-[#E23744]/20 group relative overflow-hidden"
                  >
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <ChevronRight className="h-4 w-4 text-slate-300 group-hover:text-[#E23744] transition-colors" />
                    </div>
                    <div>
        ...
        {!loading && overdueTasks.length > 0 && (
          <section className="px-4 animate-slide-up" style={{ animationDelay: '0.15s' }}>
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => navigate('/pwa/tasks')} className="text-xs text-[#E23744] font-semibold flex items-center gap-0.5">
        ...
        <section className="px-4 animate-slide-up" style={{ animationDelay: '0.2s' }}>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
              <CalendarClock className="h-4.5 w-4.5 text-[#E23744]" />
              My Follow-ups
            </h2>
            <button onClick={() => navigate('/pwa/follow-ups')} className="text-xs text-[#E23744] font-semibold flex items-center gap-0.5">
              View All <ChevronRight className="h-3.5 w-3.5" />
            </button>
          </div>
        ...
        <section className="px-4 animate-slide-up" style={{ animationDelay: '0.25s' }}>
          <h2 className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5 mb-3">
            <Activity className="h-4.5 w-4.5 text-[#E23744]" />
            Recent Active Tasks
          </h2>
                  ...
                  return (
                    ...
                    >
                      <div className={`h-8 w-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                        isTaskCompleted ? 'bg-emerald-50' : isTaskOverdue ? 'bg-red-50' : 'bg-rose-50'
                      }`}>
                      <Clock className={`h-4 w-4 ${
                        isTaskCompleted ? 'text-emerald-600' : isTaskOverdue ? 'text-red-500' : 'text-[#E23744]'
                      }`} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-slate-800 truncate">{task.title}</p>
                        <p className="text-[11px] text-slate-400 mt-0.5 font-medium">
                          Progress: <span className="text-[#E23744] font-bold">{task.completionPercent || 0}%</span> · <span className="capitalize font-semibold">{task.status}</span>
                        </p>
                      </div>
                    </div>
                  );
        ...
        <section className="px-4 animate-slide-up" style={{ animationDelay: '0.3s' }}>
          <h2 className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5 mb-3">
            <Folder className="h-4.5 w-4.5 text-[#E23744]" />
            My Project Progress
          </h2>
                    ...
                    <div className="space-y-3">
                      <div>
                        <div className="flex justify-between text-[10px] mb-1 font-semibold text-slate-500">
                          ...
                          <span className="text-[#E23744]">{project.completionPercent || 0}%</span>
                        </div>
                        <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                          <div className="h-full bg-[#E23744] rounded-full transition-all duration-500" style={{ width: `${project.completionPercent || 0}%` }} />
                        </div>
                      </div>
                      ...
                      <div>
                        <div className="flex justify-between text-[10px] mb-1 font-semibold text-slate-500">
                          ...
                          <span className="text-rose-600">{myCompletion}%</span>
                        </div>
                        <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                          <div className="h-full bg-rose-500 rounded-full transition-all duration-500" style={{ width: `${myCompletion}%` }} />
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        <section className="px-4 animate-slide-up" style={{ animationDelay: '0.35s' }}>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
              <Wrench className="h-4.5 w-4.5 text-[#E23744]" />
              My Assigned Tools
            </h2>
