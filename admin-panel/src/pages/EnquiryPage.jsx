...
function EnquiryModal({ onClose, onSubmit, members, editing }) {
 ...
 <input className="input-field" placeholder="Company name" value={form.companyName} autoComplete="new-company" onChange={e => setForm({...form, companyName: e.target.value})} /></div>
 <div><label className="label">Contact Person</label>
 <input className="input-field" placeholder="Contact person" value={form.contactPerson} autoComplete="new-contact" onChange={e => setForm({...form, contactPerson: e.target.value})} /></div>
 </div>
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
 <div><label className="label">Phone Number</label>
 <input className="input-field" placeholder="+91 XXXX XXXXXX" value={form.contactPhone} autoComplete="new-phone" onChange={e => setForm({...form, contactPhone: e.target.value})} /></div>
 ...
 <input className="input-field" placeholder="Walk-in, Phone, Email, Website..." value={form.source} autoComplete="off" onChange={e => setForm({...form, source: e.target.value})} /></div>
...
function EnquiryDetailPanel({ enquiry, onClose }) {
 ...
 <div className="relative bg-[var(--bg-card)] w-full sm:max-w-md h-full shadow-2xl overflow-y-auto border-l border-gray-100" style={{ animation: 'slideInRight 0.3s ease-out' }}>
...
export default function EnquiryPage() {
 ...
 <div className="space-y-6 page-transition">
  ...
  <div className="flex items-end gap-2 px-4 min-w-[500px] sm:min-w-0">
  ...
  />
  <p className="text-[9px] sm:text-[10px] text-[var(--text-muted)] mt-2 font-medium truncate w-full">{stage.label}</p>
  </div>
  );
  })}
  </div>
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:flex lg:items-center gap-3 w-full">
  <div className="relative w-full lg:max-w-xs">
  <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
  <input type="text" placeholder="Search company, contact..." className="input-field pl-9 w-full"
  value={search} onChange={e => setSearch(e.target.value)} />
  </div>
  <select className="input-field w-full lg:w-32" value={filterStatus} onChange={e => setFilterStatus(e.target.value)}>
  <option value="All">All Status</option>
  <option value="open">Open</option><option value="overdue">Overdue</option><option value="closed">Closed</option>
  </select>
  <select className="input-field w-full lg:w-32" value={filterType} onChange={e => setFilterType(e.target.value)}>
  <option value="All">All Types</option>
  {ENQUIRY_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
  </select>
  <select className="input-field w-full lg:w-32" value={filterMember} onChange={e => setFilterMember(e.target.value)}>
  <option value="All">All Members</option>
  {activeMembers.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
  </select>
  <div className="flex items-center gap-2 w-full lg:w-auto">
  <input type="date" className="input-field flex-1 min-w-0 lg:w-32" value={dateRange.start} onChange={e => setDateRange({...dateRange, start: e.target.value})} title="Target Start Date" />
  <span className="text-gray-400">-</span>
  <input type="date" className="input-field flex-1 min-w-0 lg:w-32" value={dateRange.end} onChange={e => setDateRange({...dateRange, end: e.target.value})} title="Target End Date" />
  </div>
 ...
 <>
  ...
  {filtered.map(e => (
  <div key={e.id} className="card p-4 space-y-3 hover:border-teal-100 transition-colors" onClick={() => setSelectedEnquiry(e)}>
  <div className="flex justify-between items-start gap-2">
  <div className="max-w-[70%]">
  <p className="font-semibold text-gray-900 text-sm break-words">{e.companyName || 'Untitled enquiry'}</p>
  <p className="text-xs text-gray-400 truncate mt-0.5">{e.contactPerson ? `${e.contactPerson} | ` : ''}{e.contactPhone || 'No phone'}</p>
  </div>
  <div className="flex flex-col items-end gap-1">
  <span className={`badge ${statusColors[e.status] || 'badge-gray'}`}>{e.status}</span>
  {(e.overdueDays || 0) > 0 && <span className="text-[10px] text-red-500 font-medium">{e.overdueDays}d overdue</span>}
  </div>
  </div>
  ...
  <span className="font-medium text-gray-700">{e.taskType || '-'}</span>
  </div>
  <div>
  <span className="block text-[10px] text-gray-400 uppercase font-medium">Assigned</span>
  <span className="font-medium text-gray-700">{e.assignedToName || '-'}</span>
  </div>
  <div>
  <span className="block text-[10px] text-gray-400 uppercase font-medium">Target Date</span>
  <span className="font-medium text-gray-700">{formatDate(e.targetDate)}</span>
  </div>
  <div>
  <span className="block text-[10px] text-gray-400 uppercase font-medium">Next Followup</span>
  <span className="font-medium text-gray-700">{formatDate(e.nextFollowupDate)}</span>
  </div>
  </div>
  ...
  <button onClick={() => setSelectedEnquiry(e)} className="p-2 rounded-lg hover:bg-gray-100 text-gray-500" title="View details">
  <Eye className="w-4 h-4" />
  </button>
  <button onClick={() => { setEditing(e); setShowModal(true); }} className="text-xs text-teal-600 hover:bg-teal-50 px-3 py-1.5 rounded-lg border border-teal-100 font-medium">Edit</button>
  <DeleteButton onClick={() => deleteEnquiryRecord(e.id, e.taskType || 'general')} />
  </div>
  </div>
  </div>
