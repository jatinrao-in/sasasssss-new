...
export default function PaymentsPage() {
 ...
 return (
  ...
  className={`w-full sm:w-auto px-3 py-2 rounded-lg text-xs font-medium border transition-all ${showAging ? 'bg-amber-50 border-amber-200 text-amber-700' : 'border-gray-200 text-[var(--text-muted)] hover:border-gray-300'}`}>
  Aging View
  </button>
  ...
  />
  <div>
  <h4 className="font-semibold text-gray-900">{p.customerName}</h4>
  <p className="text-xs text-gray-400">Invoice: {p.invoiceNumber || '-'}</p>
  </div>
  </div>
  <span className={`badge ${statusColors[p.paymentStatus] || 'badge-gray'}`}>{p.paymentStatus}</span>
  </div>
  ...
  <p className="font-semibold text-red-500">{formatCurrency(pending)}</p>
  </div>
  </div>
  ...
  <p className="mt-0.5">
  Due: {formatDate(p.targetPaymentDate)}
  {(p.overdueDays || 0) > 0 && <span className="text-red-500 font-semibold ml-1">({p.overdueDays}d overdue)</span>}
  </p>
  </div>
  <div className="flex items-center gap-2">
  <button onClick={() => setShowReceipt(p)} className="p-2 rounded-lg bg-gray-50 hover:bg-gray-100 text-gray-600" title="Receipt">
